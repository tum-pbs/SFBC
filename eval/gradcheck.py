...
def _get_notallclose_msg(analytical, numerical, output_idx, input_idx, complex_indices,
    ...
    global globalAnalytical, globalNumerical
    
    globalAnalytical = analytical
    globalNumerical = numerical
    
    return prefix + 'Jacobsian %smismatch for output %d with respect to input %d,\n' \
...
def _slow_gradcheck(func, func_out, tupled_inputs, outputs, eps, rtol, atol, check_grad_dtypes,
    ...
    # globalAnalytical = analytical_forward
    # globalNumerical = numerical
...
# fluidFeatures = torch.rand((fluidPositions.shape[0], fin))
...
# weight = Parameter(torch.Tensor(nx,ny, fluidFeatures.shape[1], fout))
...
# convOp = lambda x: convolution(fluidEdgeIndex, fluidFeatures.type(torch.float64), fluidEdgeLengths.type(torch.float64), None, x, 
#                                             fluidFeatures.shape[0], 0,
#                                         [nx, ny] , ['linear', 'linear'], [False, False], 
#                                         1,1)
