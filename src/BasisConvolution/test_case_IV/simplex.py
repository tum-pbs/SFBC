...
def getSimplexNoisePeriodic3(s, t, u,res, perm, perm_grad_index3 = None):   
    noise = np.zeros((s.shape[0], t.shape[0], u.shape[0]))
    frequency = 1
    amplitude = 1
    for ix in prange(s.shape[0]):
        for iy in prange(t.shape[0]):
            for iz in prange(u.shape[0]):
                noise[ix,iy,iz] += amplitude * _noise3(frequency * res * ix, frequency * res * iy, frequency * res * iz, perm, perm_grad_index3 = perm_grad_index3)
    return noise
    

# @njit(cache=True, parallel = True)
def getOctaveSimplexNoisePeriodic3(s, t, u, res, octaves = 2, lacunarity = 2, persistence = 0.5, seed = 1234, perm_grad_index3 = None):
    perm, _ = _init(seed)   
    noise = np.zeros((s.shape[0], t.shape[0]))
    frequency = 1
    amplitude = 1
    for _ in range(octaves):
        noise += amplitude * getSimplexNoisePeriodic3(s,t,u, res * frequency, perm)
